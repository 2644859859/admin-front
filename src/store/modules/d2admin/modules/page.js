...
export default {
  ...
  mutations: {
    ...
    initMenu (state, routes) {
      // 加载菜单
      httpPost("menu/getTree" , {}).then(res => {
          let menuData = res.result;
          if (menuData === null || menuData === "" || menuData === undefined) {
              return;
          }
          this.commit('d2admin/menu/menuSet', JSON.parse(JSON.stringify(menuData)));

          this.commit('d2admin/menu/asideSet', menuData)
          for( let i of menuData[0].subMenu){
              //i.subMenu = undefined
          }
          this.commit('d2admin/menu/headerSet', menuData[0].subMenu)
